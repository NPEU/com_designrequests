 ...
 */
class DesignRequestsRouter extends JComponentRouterBase
{
    /**
     * Build the route for the com_designrequests component
     *
    ...
    public function build(&$query)
    {
        ...
        if (
            !empty($query['view']) && $query['view'] == 'designrequests'
         && !empty($query['Itemid'])
        ) {
            unset ($query['view']);
            return $segments;
        ...
        if (
            isset($query['view']) && $query['view'] == 'designrequest'
        ...
        if (isset($query['task']) && isset($query['id']) && $query['task'] == 'designrequest.edit') {
            $segments[] = 'edit';
            $segments[] = $query['id'];
            unset ($query['task']);
            unset ($query['id']);
        ...
        if (isset($query['task']) && isset($query['id']) && $query['task'] == 'designrequest.view') {
            $segments[] = 'view';
            $segments[] = $query['id'];
            unset ($query['task']);
            unset ($query['id']);
...
/**
 * DesignRequests router functions
 *
 ...
 */
function designrequestsBuildRoute(&$query)
{
    $router = new DesignRequestsRouter;
 ...
 * @deprecated  4.0  Use Class based routers instead
 */
function designrequestsParseRoute($segments)
{
    $router = new DesignRequestsRouter;
